...
function kpis() {
  ...
  const leaveDays = db.documents.filter(d => d.type === "LEAVE" && d.status === "GENERATED")
  ...
  const byPortfolio = db.portfolios.map(p => ({ name: p.name,
  ...
  return { headcount, payroll, turnover, absenteeism, leaveDays, buckets, byPortfolio, byContract, byCategory,
    generatedDocs: db.documents.filter(d => d.status === "GENERATED").length };
}
...
function evaluation() {
  ...
  for (const u of db.users) {
    if (u.role === "GPF") {
      const mine = db.documents.filter(d => d.createdById === u.id);
      const submitted = mine.length;
      const rejectedDocs = mine.filter(d => d.steps?.some(s => s.decision === "REJECTED"));
      const reasons = {};
      ...
      rows.push({ user: u.fullName, role: "GPF", submitted,
        ...
        resubmissions: mine.reduce((s, d) => s + Math.max(0, (d.cycle || 1) - 1), 0),
...
function compliance() {
  ...
  for (const e of db.employees) {
    ...
    const pf = db.portfolios.find(p => p.id === e.portfolioId);
    const uploaded = new Set(db.files.filter(f => f.employeeId === e.id).map(f => f.docType));
    ...
    const med = db.files.find(f => f.employeeId === e.id && f.docType === "XVI");
  ...
  const score = db.employees.length
...
const REPORTS = {
  ...
  headcount: { label: "Effectifs par portefeuille & catégorie", build: () => {
    const k = kpis();
...
router.get("/:key", allow("CD", "RJ", "ADM"), (req, res) => {
  ...
  res.json({ key: req.params.key, label: r.label, data: r.build() });
});
